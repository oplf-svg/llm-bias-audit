...
def plot_agreement(agree_df: pd.DataFrame, out_dir: Path):
    ...
    ax.bar(positions, agree_df.spearman_rho,
           yerr=[agree_df.spearman_rho - agree_df.spearman_ci95_lo,
                 agree_df.spearman_ci95_hi - agree_df.spearman_rho],
           color="#1F3864", alpha=0.8, capsize=4)
...
def plot_model_by_benchmark(harmonised: pd.DataFrame, out_dir: Path):
    ...
    sns.heatmap(wide, annot=True, fmt=".2f", cmap="RdBu_r", center=0, ax=ax,
